---
layout: page
title: Working Groups
permalink: /about-us/working-groups
---


<p>Working Groups meet every two months to hear from partners, share and develop good practice, and offer advice. Working Groups also develop collaboration among partners (e.g. funding bids, sharing resources), create tool documentation (e.g. tutorials) and share pedagogical ideas. We also welcome guest speakers from outside academia, such as from journalism or commerce.</p>
<p>Each Working Group is run by two Coordinators elected from the Partners of that Activity. The Coordinators from all the Working Groups meet once a month.</p>
<p>If you would like to join a particular group, please contact the Coordinators via the relevant email link below.</p>

<hr />

<h2>Annotation</h2>
<h3>The use of semantic annotation to link and explore historical place information. See our <a href="https://docs.google.com/document/d/1Id6Ml9XKxiyf_VsG98DyeXzlAtLUbKqfhiNRkFDY4PY/edit">Charter</a>.</h3>    
<p>Coordinators: <a href="mailto:annotation@pelagios.org">Anne Chen and Elton Barker</a></p>

<hr />

<h2>Maps</h2>
<h3>Establishes core requirements for global authority files on historic places and their alignment.</h3> <p>See our <a href="https://docs.google.com/document/d/1NyIrVYX6ThMhykyxgV7H_HJud2gwkpPGtj2PZVDcXi8/edit">Charter</a>.</p>
<p>Coordinators: <a href="mailto:gazetteers@pelagios.org">Tom Gheldof and Valeria Vitale</a></p>

<hr />

<h2>People</h2>
<h3>Works towards consensus and standards for common representation of historical people in structured Linked Open Data.</h3>
<p>Rather than replacing existing solutions around shared person data formats, we recommend lightweight, minimal standards for interchange data, compatible with Pelagios registries, APIs and annotation tools. Needs and solutions will arise from consultation with communities of practice and stakeholders, in close collaboration with other PN activities. Results and discussions are disseminated openly throughout via online platforms, forums and repositories.
<p>Coordinators: <a href="mailto:people@pelagios.org">Gabriel Bodard and Jun Ogawa</a></p>


<hr />

<h2>Registry</h2>
<h3>Establishes services for registering and discovering linked data collections for places.</h3>
<p>Coordinators: <a href="mailto:registry@pelagios.org">Leif Isaksen and Pascal Belouin</a></p>

<hr />

<h2>Time</h2>
<h3>Finding consensus on the representation of historical time in structured data.</h3>
<p>Coordinators: <a href="mailto:time@pelagios.org">Ryan Shaw and Adam Rabinowitz</a></p>

<hr />

<h2>Visualisation</h2>
<h3>Support developers and end-users in conceptualising and developing methods and tools for:</h3>
<ul>
    <li>visualisation, such as showing the geographic scope of a dataset, or representing a spatial network;</li>
    <li>discovery, including the search and retrieval of data from across the collective of Partners, using filters that include period, data type, geographic location, and source;</li>
    <li>analysis, by leveraging the outcome of linked data to stimulate new research questions and provide new insights.</li>
</ul>
<p>Coordinators: <a href="mailto:visualisation@pelagios.org">Gethin Rees and Stephen Gadd</a></p>




